...
* [Numpy](http://www.numpy.org/), [SciPy](https://www.scipy.org/), [Pandas](http://pandas.pydata.org/), [Scikit Learn](http://scikit-learn.org/), [Matplotlib](http://matplotlib.org/), [Flask](http://flask.pocoo.org/)
...
To build the images locally. You will need to enter the following commands.

```bash
git clone https://github.com/DevoteamDigitalFactory/ddf-facerecognition-backend.git
cd ddf-facerecognition-backend
docker build -t ddfmldev:dev -f Dockerfile.dev .
```
...
There are three ways to run the Docker

** Interactive bash **
This mode is helpfull i you plan to modify te environment and add or vhange the depedecies installed inside the Docker. It is also helpful to test bash command
```bash
docker run -it -p 8888:8888 -p 5000:5000 -p 6006:6006 --rm ddfmldev:dev /bin/bash
```

** Jupyter Notebook **
This mode will run you Docker has a Jupyter notebook [http://jupyter.org/], it will allow you to access the container simulating the python environment as an interactive shell. Tis shell will be accessible over the web
```bash
docker run -it -p 8888:8888 -p 5000:5000 -p 6006:6006 --rm jupyter notebook --allow-root
```

** Web App **
This will run the main script starting the Flask web app accessible on the port 5000 of your local machine.
 ```bash
docker run -it -p 8888:8888 -p 5000:5000 -p 6006:6006 --rm python main.py
```
